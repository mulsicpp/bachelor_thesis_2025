...
We create one buffer for each usage type of our data. We have one buffer for the vertex indices, one for static vertex positions of the mesh and one for dynamic vertex positions, that may be different for each mesh. An attribute can then reference a sub-subsection into one of these buffers using a sub-buffer. A sub-buffer has a reference to the buffer of intrest using a \code{ptr::Shared<Buffer>}, as well as an offset and a length. We do not need to store the type of the elements in the sub-buffer, since the type is fixed and can be determined by the variable. We know that the sub-buffers \code{positions} in \code{Mesh} and \code{dynamic\_positions} in \code{Node} store a sequence of \code{vec3}s, while the sub-buffer \code{indices} in \code{Mesh} stores a sequence of \code{unsigned int}s.
