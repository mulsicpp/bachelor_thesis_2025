...
The \code{Mesh} class represents a glTF mesh. It stores an array of \code{Primitive}s. Each \code{Primitive} stores three sub-buffers: \code{positions} for the vertex positions, \code{indices} for the vertex indices and \code{joint\_weights} for the joint weights used for skinning the mesh. The positions and indices are pretty straight forward and just store a sequence of \code{vec3}s and \code{unsigned int}s respectively. The joint weights are a bit more complex, because they are stored in a 2D array of \code{JointWeight} structs. A \code{JointWeight} contains the \code{index} of the node in the skeleton of the skin and its \code{weight}. All the arrays of joint weights for a vertex are stored in sequence like shown in this figure:
...
Additionally, each \code{Primitive} also contains the member variables \code{positions\_cpu} and \code{joint\_weights\_cpu}, which store the same data as \code{positions} and \code{joint\_weights}, but this time inside \code{std::vector}s for easier access in CPU skinning.

Finally, a \code{Mesh} can also contain a \code{Blas}, which is created for the static mesh. It is only created if a mesh is used statically by a node, i.e. a node that contains the mesh without a skin.
...
A node also stores an array of child nodes and it can also contain a \code{Mesh}. If a mesh is present, it also stores a \code{Blas} and the node must generate a \code{TlasInstance} that references the node's \code{Blas} when building the acceleration structure. After building the acceleration structure for the first time, the node also stores the index of the \code{TlasInstance}, which is necessary to update the \code{TlasInstance} later. If the node does not have a \code{Skin}, the node's \code{Blas} is the the same as the one inside the mesh.

If the node does however contain a \code{Skin}, it stores an array of sub-buffers for dynamic positions. For every primitive of the node's \code{Mesh}, a sub-buffer for dynamic positions needs to be present in the array. Each sub-buffer needs to have the same size as the sub-buffer for the static positions of the respective primitive. These \code{dynamic\_positions} are used to store the vertex positions of a mesh after skinning. If a \code{Skin} is present, the \code{Blas} is not simply just a reference the meshes' \code{Blas}, but it is a separate, dynamic \code{Blas}, which can be updated with the node's \code{dynamic\_positions}.

\subsubsection{The \code{Animation} class}

The \code{Animation} class represents a glTF animation. It contains an array of \code{AnimationSampler}s, which contain a reference to the node, that they modify during animation, and also the virtual function \code{apply\_for}. This function samples a value at the specified point in time and applies it to a certain property of the nodes transform. There are three sub-classes of \code{AnimationSampler}, which override the \code{apply\_for} function:
...
We create one buffer for each type of data: one for vertex indices, one for joint weights, one for static vertex positions and one for dynamic vertex positions. An attribute can then reference a sub-subsection into one of these buffers using a sub-buffer. A sub-buffer has a reference to the buffer of intrest using a \code{ptr::Shared<Buffer>}, as well as an offset and a length. We do not need to store the type of the elements in the sub-buffer, since the type is fixed and can be determined by the variable. For example, we know that the sub-buffer \code{positions} in a \code{Mesh} stores a sequence of \code{vec3}s.
