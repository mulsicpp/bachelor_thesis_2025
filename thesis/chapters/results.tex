...
We can see the expected outcome of \code{PREFER\_FAST\_BUILD} having shorter build durations, but longer trace durations compared to \code{PREFER\_FAST\_TRACE}. It is important to mention that these flags do not impact refitting. These flags effect the structure of the BVH, which refitting does not touch.

Whether you should use \code{PREFER\_FAST\_BUILD} or \code{PREFER\_FAST\_TRACE} depends on the scenario. If you are tracing a lot of rays and the trace duration is the bottleneck in your application, you should probably use \code{PREFER\_FAST\_BUILD}. On the other hand, if most of your time is spent on rebuilding (not refitting!), \code{PREFER\_FAST\_TRACE} is most likely the better choice.